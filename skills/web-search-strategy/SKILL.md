...
|---------|:------:|-----------------|
| **ddgs** (current) | ✅ Active | Free, no key. ~25% empty-result rate. 3-5 results. Add 3-5s delay between calls. Persisted via PYTHONPATH → `/opt/data/pip-packages/`. Not in LAZY_DEPS (ddgs is the only search backend uncovered by Hermes' auto-installer). |
| **SearXNG** | ✅ Standby | Self-hosted Docker at searxng-core:8080. bing + mojeek engines, 17-20 results/query, no rate limit. Switch via `hermes config set web.search_backend searxng`. |
...
**Delay rule**: when using ddgs, add 3-5 seconds between successive web_search
calls. Reduces the ~25% empty-result rate from DuckDuckGo rate-limiting.
...
For complex tasks, design your search plan BEFORE executing:
...
Report progress between rounds: "Phase 1 done — found 12 candidates across 3
dimensions. Now deep-reading the top 4."
