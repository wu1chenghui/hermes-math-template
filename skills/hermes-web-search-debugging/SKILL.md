...
Settings path: `/workspace/docker-data/searxng/config/settings.yml` (Docker volume
mounted from the Hermes docker-compose). Also reachable from host at the same path
...
**After editing**: `cd /workspace && docker compose restart searxng-core`
...
Our setup: SearXNG runs as `searxng-core` container defined in `/workspace/docker-compose.yml`.
Docker network URL: `http://searxng-core:8080` (from inside Hermes container).
Host-mapped URL: `http://localhost:8085` (from WSL host).
Settings: `/workspace/docker-data/searxng/config/settings.yml`.
